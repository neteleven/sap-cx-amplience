...
package de.neteleven.amplience.addon.jalo;

import de.hybris.platform.core.Registry;
import de.hybris.platform.util.JspContext;
import de.neteleven.amplience.addon.constants.AmplienceaddonConstants;
...
/**
 * This is the extension manager of the Amplienceaddon extension.
 */
public class AmplienceaddonManager extends GeneratedAmplienceaddonManager {
    /**
     * Edit the local|project.properties to change logging behavior (properties 'log4j.*').
     */
    private static final Logger LOG = Logger.getLogger(AmplienceaddonManager.class.getName());
     ...
     */
    public static AmplienceaddonManager getInstance() {
        return (AmplienceaddonManager) Registry.getCurrentTenant().getJaloConnection().getExtensionManager().getExtension(
                AmplienceaddonConstants.EXTENSIONNAME);
    }
     ...
     */
    public AmplienceaddonManager() {
        if (LOG.isDebugEnabled()) {
            LOG.debug("constructor of AmplienceaddonManager called.");
        }
    }
    ...
    public void init() {
        if (LOG.isDebugEnabled()) {
            LOG.debug("init() of AmplienceaddonManager called. " + getTenant().getTenantID());
        }
    }
    ...
    public void destroy() {
        if (LOG.isDebugEnabled()) {
            LOG.debug("destroy() of AmplienceaddonManager called, current tenant: " + getTenant().getTenantID());
        }
    }
